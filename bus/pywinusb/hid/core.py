...
class HidReport(object):
    ...
    def report_size(self):
        """Retreive asociated report size value"""
        return self.__raw_report_size
    ...
    def set_raw_data(self, raw_data):
        ...
        if len(raw_data) != self.__raw_report_size:
            raise HIDError( "Report size has to be %d elements (bytes) current %d" \
                % (self.__raw_report_size, len(raw_data)) )
    ...
    def send(self, raw_data = None):
        ...
        if raw_data and (len(raw_data) != self.__raw_report_size):
            raise HIDError("Report size has to be %d elements (%d bytes)" \
                % (self.__raw_report_size, len(raw_data)))
